...
const DisputeDetailPage = () => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const result = await scDriver.applyADM(data.disputeId, decision);
            ...
            const { data } = await bkdDriver.getDisputeById(disputeId);
            setData(data);
        }
    };
    ...
    const getReputation = async () => {
        if (scDriver && data) {
            const reputationPick = await scDriver.getMerchantReputation(data.order.product.merchant.walletAddress);
            console.log('reputationPick', reputationPick);
            // const reputationPick = await escrowContract.methods
            //     .getMerchantReputation(
            //         data.order.product.merchant.walletAddress
            //     )
            //     .call();
            setReputation(Number(reputationPick.toString()));
        }
    };
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            setData(data);
            // const agentData = await bkdDriver.agents({ status: 'waiting' });
            // setAgents(agentData);
        };
        fetchData();
    }, [web3, connected, history, bkdDriver]);

    useEffect(() => {
        getReputation();
    }, [getReputation, scDriver]);
